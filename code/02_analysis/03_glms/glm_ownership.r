...
naive_model <- glm(hs ~ own_type + hdw_scaled + avg_fuel_moisture_scaled + cwd_scaled +
                          slope_scaled + tpi_scaled + heat_load_scaled + prev_sev_scaled + fire_name,
                  data = data[s,], family = binomial(), model = FALSE, y = FALSE)
summary(naive_model)
saveRDS(naive_model, "data/model_objects/naive_model_ownership.rds")
...
saveRDS(bb, "data/autocor_scale/autocor_scale_m_ownership.rds")

bb <- readRDS("data/autocor_scale/autocor_scale_m_ownership.rds")
...
run_bootstrap <- function(iter) {
  ...
  bs.glm <- speedglm(hs ~ own_type + hdw_scaled + avg_fuel_moisture_scaled + cwd_scaled +
                          slope_scaled + tpi_scaled + heat_load_scaled + prev_sev_scaled + fire_name,
                      data = bs.data,
                      family = binomial(),
                      y = FALSE,
...
for(i in 1:1) {
  ...
  write.csv(bs.coefs, file = "data/bootstraps/bootstrap_coefs_ownership.csv")
...
write.csv(bs.coefs, file = "data/bootstraps/bootstrap_coefs_ownership.csv", row.names = FALSE)

## this is the janky way to force R to restart after each iteration
## first save workspace image
save.image(file = "bsdownership.Rdata")
## then run external file (contents below)
source("code/02_analysis/run_bootstrap_ownership.R")
