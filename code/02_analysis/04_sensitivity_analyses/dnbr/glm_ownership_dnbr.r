...
set.seed(1211) ## my bday
s <- sample(1:nrow(data), round(0.25 * nrow(data)))

naive_model_dnbr <- glm(hs_dnbr ~ own_type + hdw_scaled + avg_fuel_moisture_scaled + cwd_scaled +
...
saveRDS(naive_model_dnbr, "data/model_objects/naive_model_ownership_dnbr.rds")
...
saveRDS(bb, "data/autocor_scale/autocor_scale_m_ownership_dnbr.rds")
...
run_bootstrap <- function(iter) {
  ...
  bs.glm <- speedglm(hs_dnbr ~ own_type + hdw_scaled + avg_fuel_moisture_scaled + cwd_scaled +
                          slope_scaled + tpi_scaled + heat_load_scaled + prev_sev_scaled + fire_name,
                      data = bs.data,
                      family = binomial(),
                      y = FALSE,
...
for(i in 1:1) {
  ...
  write.csv(bs.coefs, file = "data/bootstraps/bootstrap_coefs_ownership_dnbr.csv")
...
write.csv(bs.coefs, file = "data/bootstraps/bootstrap_coefs_ownership_dnbr.csv", row.names = FALSE)

## this is the janky way to force R to restart after each iteration
## first save workspace image
save.image(file = "bsdownership_dnbr.Rdata")
source("code/02_analysis/run_bootstrap_ownership_dnbr.R")
