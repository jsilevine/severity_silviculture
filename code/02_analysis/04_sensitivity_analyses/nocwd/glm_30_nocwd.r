...
naive_model <- glm(hs ~ hdw_scaled + avg_fuel_moisture_scaled + prev_sev_scaled + 
                     ...
                     slope_scaled + tpi_scaled + heat_load_scaled +
                     fire_name,
                     data = data[s,], family = binomial(), model = FALSE, y = FALSE)

summary(naive_model)

saveRDS(naive_model, "data/model_objects/naive_model_structure_nocwd.rds")

bb <- readRDS("data/autocor_scale/autocor_scale_m.rds")
...
run_bootstrap <- function(iter) {
  ...
  bs.glm <- speedglm(hs ~ hdw_scaled + avg_fuel_moisture_scaled + prev_sev_scaled + 
                       mean_dens_30_scaled + mean_dens_30_scaled:hdw_scaled + 
                       clust_30_scaled + 
                       mean_ht_30_scaled + mean_ht_30_scaled:hdw_scaled + 
                       mean_area_30_scaled + mean_area_30_scaled:hdw_scaled +
...
write.csv(bs.coefs, file = "data/bootstraps/bootstrap_coefs_30_nocwd.csv")

## this is the janky way to force R to restart after each iteration
## first save workspace image
save.image(file = "bsd_nocwd.Rdata")
## then run external file (contents below)
source("code/02_analysis/04_sensitivity_analyses/nocwd/run_bootstrap_30_nocwd.r")
