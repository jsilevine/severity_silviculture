...
northcomplex_perim <- readRDS("data/perimeters/northcomplex_perim.rds")
northcomplex_start <- as.Date(northcomplex_perim$ALARM_DATE)
northcomplex_end <- as.Date(northcomplex_perim$CONT_DATE)
northcomplex_severity <- raster("data/rdnbr/northcomplex_rdnbr.tif")

northcomplex_daily_burned <- extract_daily_perims(start_date = northcomplex_start, end_date = northcomplex_end,
                                                  perim = northcomplex_perim, severity_raster = northcomplex_severity,
                                                  viirs = viirs, is_dixie = FALSE, cluster_tol = 4,
                                                  cluster_dist = 1125, large_cluster_dist = 10000)
plot(northcomplex_daily_burned)
saveRDS(northcomplex_daily_burned, "data/fire_progression/northcomplex_daily_burned.rds")

## write csv
northcomplex_daily_burned.df <- as.data.frame(northcomplex_daily_burned, xy = TRUE, na.rm = TRUE)
colnames(northcomplex_daily_burned.df)[3] <- "datetime"
write.csv(northcomplex_daily_burned.df, "data/fire_progression/northcomplex_daily_burned.csv")
rm(northcomplex_daily_burned.df)

sheep_perim <- readRDS("data/perimeters/sheep_perim.rds")
sheep_start <- as.Date(sheep_perim$ALARM_DATE)
sheep_end <- as.Date(sheep_perim$CONT_DATE)
sheep_severity <- raster("data/rdnbr/sheep_rdnbr.tif")

sheep_daily_burned <- extract_daily_perims(start_date = sheep_start, end_date = sheep_end,
                                                  perim = sheep_perim, severity_raster = sheep_severity,
                                           viirs = viirs, is_dixie = FALSE, cluster_tol = 4,
                                           cluster_dist = 1125, large_cluster_dist = 4000)
plot(sheep_daily_burned)
saveRDS(sheep_daily_burned, "data/fire_progression/sheep_daily_burned.rds")

## write csv
sheep_daily_burned.df <- as.data.frame(sheep_daily_burned, xy = TRUE, na.rm = TRUE)
colnames(sheep_daily_burned.df)[3] <- "datetime"
write.csv(sheep_daily_burned.df, "data/fire_progression/sheep_daily_burned.csv")
rm(sheep_daily_burned.df)
...
walker_daily_burned <- extract_daily_perims(start_date = walker_start, end_date = walker_end,
                                            ...
                                            viirs = viirs, is_dixie = FALSE, cluster_tol = 4,
                                            cluster_dist = 1125, large_cluster_dist = 4000)
plot(walker_daily_burned)
saveRDS(walker_daily_burned, "data/fire_progression/walker_daily_burned.rds")

## write csv
walker_daily_burned.df <- as.data.frame(walker_daily_burned, xy = TRUE, na.rm = TRUE)
colnames(walker_daily_burned.df)[3] <- "datetime"
write.csv(walker_daily_burned.df, "data/fire_progression/walker_daily_burned.csv")
rm(walker_daily_burned.df)


sugar_perim <- readRDS("data/perimeters/sugar_perim.rds")
sugar_start <- as.Date(sugar_perim$ALARM_DATE)
sugar_end <- as.Date(sugar_perim$CONT_DATE)
sugar_severity <- raster("data/rdnbr//sugar_rdnbr.tif")

sugar_daily_burned <- extract_daily_perims(start_date = sugar_start, end_date = sugar_end,
                                           perim = sugar_perim, severity_raster = sugar_severity,
                                           viirs = viirs, is_dixie = FALSE, cluster_tol = 4,
                                           cluster_dist = 1125, large_cluster_dist = 4000)
plot(sugar_daily_burned)
saveRDS(sugar_daily_burned, "data/fire_progression/sugar_daily_burned.rds")

# write csv
sugar_daily_burned.df <- as.data.frame(sugar_daily_burned, xy = TRUE, na.rm = TRUE)
